...
class IndexController extends HomeController {
    ...
    public function index(){
        //厂商
        $map['pid']=array('eq',22);
        $changshang=M('category')->where($map)->order('id desc')->select();
        $this->assign('changshang',$changshang);
        //选择级别
        $map['pid']=array('eq',23);
        $jibie=M('category')->where($map)->order('id desc')->select();
    ...
    public function newsList(){
        //测评通知图片
        $map['typeid']=$where['typeid']=array('eq',21);
        $map1['icon']=$where['icon']=array('neq',0);
        $map1['status']=$map['status']=$where['status']=array('eq',1);
        $img_news=M('article')->where($where)->find();
        $this->assign('img_news',$img_news);

        //测评通知3条新闻
        $map['id']=array('not in',$img_news['id']);
        $three_news=M('article')->where($map)->order('id desc')->limit(3)->select();
        $this->assign('three_news',$three_news);
        //新闻资讯
        $map1['flag']=array('eq',1);
        $map1['typeid']=array('in','14,12,13');
        $news=M('article')->where($map1)->order('id desc')->limit(3)->select();
