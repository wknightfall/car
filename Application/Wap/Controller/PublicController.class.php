...
// 		读取站点配置
	...
	public function register(){
		if(IS_POST){
			...
			if($useradd){
                ...
                session('userinfo',M('usermember')->where("id=".$useradd)->find());
	...
	public function xiugshouji(){
		if (IS_POST) {
			...
			$user = M('usermember')->where($fidn)->find();
			if($user){
				exit(json_encode(array('code'=>401,'msg'=>'手机号存在!')));
			}
	...
	public function forget(){
		if (IS_POST) {
			...
			if (M('usermember')->where("phone=".$re['phone'])->save(array('password'=>$list['newpass']))) {
				exit(json_encode(array('code' => 200,'msg'=>'修改成功' )));
			} else {
				exit(json_encode(array('code' => 400, 'msg'=>'修改失败')));
			}
	...
	/*public function callback(){
		...
		if(!empty($res)){
			...
			if(ismobile()){
				$this->redirect('Userinfo/userindex');
			}else{
				$this->redirect('Userinfo/index');
			}
		}else{
			session('wxdj_three_info',$info);
			session('three_login_type','weixin');//向前台传递三方登录类型
			$this->redirect('Public/bang'); //绑定账号
		}
	...
	/*public function qqlogin(){
		$callback = 'http://www.gzsxwljt.com/home/public/qqlogin';
		$info = D('Login')->qq_userinfo($_REQUEST['code'],$_REQUEST['state'],C('QQ_LOGIN_APPID'),C('QQ_LOGIN_APPKEY'),$callback);
		...
		if(!empty($res)){
			...
			session('wxdj_three_info',$info);
			session('three_login_type','qq');//向前台传递三方登录类型
			$this->redirect('Public/bang'); //绑定账号
		}
