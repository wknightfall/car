...
class CartypeController extends AdminController {
	...
	public function _initialize(){
		...
		$this->assign("lists",M('brand')->where($type)->order('sort asc')->select());//公司类型
    ...
    public function index(){
		...
		$lists = M('cartype')->where($where)->order('id desc')->select();
	...
	public function edit(){
		...
		if(IS_POST){
		    ...
		    if($news->create()){
				...
				$resu = M('cartype')->save($_POST);
				if($resu){
					$this->success('修改成功！',U('Cartype/index'));
					//$this->success('修改成功！',U('Cartype/index',array('typeid'=>$_POST[typeid])));
				}else{
					$this->error('修改失败！');
				}
			}else{
	...
	public function add(){
		if(IS_POST){
		    ...
		    if($news->create()){
				$_POST[flag] = implode(',',$_POST[flag]);
				$resu = M('cartype')->add($_POST);
				if($resu){
					$this->success('新增成功！',U('Cartype/index'));
					//$this->success('新增成功！',U('Cartype/index',array('typeid'=>$_POST[typeid])));
				}else{
					$this->error('新增失败！');
				}
			}else{
	...
	public function del(){
		...
		if($id>0){
		  $lists = M('cartype')->where('id='.$id)->save(array('status'=>-1));
		  if($lists){
			  $this->success('删除成功');
		  }
		}else{
