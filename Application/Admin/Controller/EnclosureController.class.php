 ...
 * 后台用户控制器
 */
class EnclosureController extends AdminController {
    ...
    public function index(){
		...
		//分页
        $listsCount = M('enclosure')->where($where)->count();
        $Page       = new \Think\Page($listsCount,20);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        $show       = $Page->show();// 分页显示输出

	    $lists = M('enclosure')->where($where)->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function edit(){
		...
		if(IS_POST){
		    ...
		    if($news->create()){
				...
				$resu = M('enclosure')->save($_POST);
				if($resu){
					$this->success('修改成功！',U('enclosure/index',array('typeid'=>$_POST[typeid])));
				}else{
					$this->error('修改失败！');
				}
			}else{
	...
	public function add(){
		if(IS_POST){
		    ...
		    if($news->create()){
				...
				$resu = M('enclosure')->add($_POST);
				if($resu){
					$this->success('新增成功！',U('enclosure/index',array('typeid'=>$_POST[typeid])));
				}else{
					$this->error('新增失败！');
				}
			}else{
